 ...
 * @author Carlos
 */
public class CMenuScreen implements IKeyPressedAndReleasedListener {
    private CSceneControl m_pSceneControl=null;
     ...
     public CMenuScreen(PhysEngRunnableGameCanvas pPhysEngRunnableGameCanvas,CSceneControl pSceneControl){
     ...
     public void run(){
         ...
         m_pPhysEngRunnableGameCanvas.write(m_pPhysEngRunnableGameCanvas.getHeight()/2, new String("This is the menu screen. Press any key to continue"));
         m_lCount++;
...
//             m_pSceneControl.setState(CSceneControl.PLAYING_STAGE_SCREEN);
     ...
     public void drawBackground(){
        m_pPhysEngRunnableGameCanvas.m_g.setColor(100, 0, 0);
     ...
     public void keyPressedAndReleased(int keyCode){ 
        if(m_lCount>20){
             if(m_pSceneControl.getState()==CSceneControl.MENU_SCREEN)
             {
                 m_bIsFinished=true;
